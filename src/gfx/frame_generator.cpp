...
namespace gfx
{
    ...
    void FrameGenerator::renderFrame(FrameGenerateArgs generators, gfx::Camera camera)
    {
        const u32 graphicsQueueIndex =
            this->renderer->getDevice()->getFamilyOfQueueType(gfx::core::vulkan::Device::QueueType::Graphics).value();
        ...
        this->has_resize_ocurred = this->renderer->recordOnThread(std::move(rec));
    }
} // namespace gfx